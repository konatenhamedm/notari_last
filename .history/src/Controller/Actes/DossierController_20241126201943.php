 ...
 */
    ...
    public function piece(
       ...
       /* dd($dossier->getPieces()); */
        ...
        $oldPieces = $dossier->getPieces();
         ...
         if (!$dossier->getPieces()->count()) {
          ...
          /*   foreach ($dossier->getIdentifications() as $key => $value) {
            ...
            foreach ($dossier->getIdentifications() as $key => $value) {
                foreach ($identificationRepository->findBy(['client'=>$value->getClients()]) as $key => $doc) {
                    $piece = new Piece();

                    $piece->setAttribut($value->getAttribut());
                    $piece->setClient($value->getClients());
                    //$piece->setPath($doc->get());
                    $dossier->addPiece($piece);
                }
        ...
        foreach ($dossier->getIdentifications() as $pieces) {
          foreach ($identificationRepository->findAllBy(['clients' => $pieces->getAttribut()]) as $key => $value) {
            # code...
            $pieces = new Piece();
            $pieces->setAttribut($value->getAttribut());
            $pieces->setClient($value->getClients());
            $dossier->addPiece($pieces);
          }
